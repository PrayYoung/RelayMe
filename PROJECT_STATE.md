...
- v0.4 executor-task bridge is locally and real-host accepted for the isolated non-provider profile. Launcher compatibility is now proven offline, but provider acceptance remains blocked because the one authorized task exposed a temporary-profile base-revision registration error before container launch.

## Next action

- Before any fresh provider authorization, correct the deployment-local profile to bind its own fixed source base revision, then repeat one exact-host acceptance without changing RelayMe Core or policy.

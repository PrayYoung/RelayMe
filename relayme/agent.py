    ...
    def start_registered_executor_task(self, arguments: dict[str, Any], execution_id: str) -> dict[str, Any]:
        ...
        with self._execution_lock:
            ...
            try:
                ...
                result = {"execution_id": execution_id, "task_id": execution_id, "state": state, "executor_profile_id": profile_id, "task_spec_id": "patch-and-test", "task_identity": profile["task_identity"], "base_revision": base_revision, "summary": summary, "changed_files": changed, "tests_run": tests, "warnings": warnings, "suggested_reviewer_evidence": [item["name"] for item in artifacts], "started_at": started_at, "finished_at": int(time.time()), "duration_ms": int((time.monotonic() - started) * 1000), "timeout_seconds": profile["timeout_seconds"], "exit_code": exit_code, "terminating_signal": signal_number, "stdout": output["stdout"]["text"], "stderr": output["stderr"]["text"], "stdout_bytes": output["stdout"]["bytes"], "stderr_bytes": output["stderr"]["bytes"], "stdout_truncated": output["stdout"]["truncated"], "stderr_truncated": output["stderr"]["truncated"], "stdout_hash": output["stdout"]["hash"], "stderr_hash": output["stderr"]["hash"], "transcript_truncated": output["stdout"]["truncated"] or output["stderr"]["truncated"], "artifacts": artifacts, "output_directory": str(task_output), "reviewable": state in {"SUCCEEDED", "FAILED", "TIMED_OUT"}}
            finally:

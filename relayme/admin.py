"""Local-only Controller administration for RelayMe scoped clients."""
...
from .controller import CAPABILITIES, EXECUTOR_CAPABILITY, R1_CAPABILITY, Store
...
def main(argv: list[str] | None = None) -> None:
    ...
    create.add_argument("--capability", action="append", required=True,
                        help=f"R0 capability or exact {R1_CAPABILITY}:TASK_ID or run_executor_task:PROFILE_ID scope")
    ...
    def valid_scope(capability: str) -> bool:
        return capability in CAPABILITIES - {R1_CAPABILITY, EXECUTOR_CAPABILITY} or (capability.startswith(R1_CAPABILITY + ":") and len(capability) > len(R1_CAPABILITY) + 1) or (capability.startswith("run_executor_task:") and len(capability) > len("run_executor_task:"))
    if args.command == "create-client" and not all(valid_scope(capability) for capability in args.capability):
        parser.error("--capability must be an R0 capability or an exact registered/executor-task scope")
    store = Store(args.db)
    try:
